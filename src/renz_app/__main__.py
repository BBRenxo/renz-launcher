...
class Client:
    ...
    def chat(self, user_msg: str, stream: bool = False, on_token=None) -> str:
        """Send a message, get a response. Handles tool calls.
        If stream=True, tokens are printed in real time.
        """
        ...
        while True:
            ...
            try:
                ...
                with urllib.request.urlopen(req, timeout=300) as resp:
                    if stream:
                        return self._parse_sse_stream(resp, on_token)
    ...
    def _parse_sse_stream(self, resp, on_token):
        """Parse an SSE stream. Returns final content or reasoning."""
        content_buf = ""
        reasoning_buf = ""
        tool_calls_buf = []
        for line in resp:
            ...
            if 'reasoning' in delta and delta['reasoning']:
                ...
                if on_token and not content_buf:
                    on_token(delta['reasoning'])
        ...
        final_content = content_buf or reasoning_buf
        ...
        # Loop back
        return self.chat("", stream=stream, on_token=on_token) if False else self._continue_tool_loop(stream, on_token)

    def _continue_tool_loop(self, stream, on_token):
        """After tool calls, send the next request."""
        while True:
            payload = {
                "model": self.model,
                "messages": [{"role": "system", "content": self.persona}] + self.history,
...
def main():
    ...
    while True:
        ...
        sys.stdout.write(f"{C.MAG}renz>{C.R} ")
        sys.stdout.flush()

        # State: skip the "ratman4080: " prefill if it appears at the start
        buf = ""
        prefix_to_strip = "ratman4080:"
        prefix_buf = ""  # accumulating to detect prefix
        prefix_done = False

        def _on_token(token):
            nonlocal buf, prefix_buf, prefix_done
            buf += token
            if not prefix_done:
                prefix_buf += token
                # Check if prefix matches
                if prefix_to_strip.startswith(prefix_buf):
                    if prefix_buf == prefix_to_strip:
                        # Now consume the next " " and we're done
                        # Actually wait for next token
                        prefix_done = True
                    return
                else:
                    # Doesn't match — flush prefix_buf + token, mark done
                    sys.stdout.write(prefix_buf)
                    prefix_done = True
            sys.stdout.write(token)
            sys.stdout.flush()
        # Strip leading "ratman4080: " if present
        response = client.chat(user_input, stream=True, on_token=_on_token)
        sys.stdout.write("\n")
        # If response is a final string (no streaming happened), print it
